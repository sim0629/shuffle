...
<script type="text/javascript" src="<?php echo base_url($swf_object) ?>"></script>
<script type="text/javascript">
    var so = new SWFObject("<?php echo base_url($swf_mp3) ?>","mediaplayer","<?php echo $mp3_width ?>","<?php echo $mp3_height ?>","7");
    so.addVariable("repeat","always");
    so.addVariable("width","<?php echo $mp3_width ?>");
    so.addVariable("height","<?php echo $mp3_height ?>");
    so.addVariable("displayheight","0");
    so.addVariable("file","<?php echo base_url($loadfile) ?>?" + (new Date().getTime()));
    so.addVariable("autostart","true");
    so.addVariable("shuffle","<?php echo $shuf ?>");
    so.addVariable("skin","<?php echo base_url('simple.swf') ?>");
